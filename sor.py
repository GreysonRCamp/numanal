...
for k in range(10):
    ...
    for i in range(len(A)):
        xnew[i] = scaledx[i] + (omega/A[i][i]) * (b[i] - summation(A[i],xnew,0,i) - summation(A[i],xold,i+1,len(xold)))
    xold = list(xnew)
...
def print_table_to_file(headers, rows, filename):
    ...
    with open(filename, "w") as f:
        f.write("https://github.com/GreysonRCamp/numanal/blob/main/sor.py \n\nSOR\n\n")
        # Header
...
# Write table to file
print_table_to_file(headers, rows, "hw3q2sor.txt")
